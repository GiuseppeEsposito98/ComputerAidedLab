...
def normal_polar_coordinates(mu, sigma):
    ...
    z1 = b * cos(theta)
    z2 = b * sin(theta)
    # here we pass from the coordinates generated from a standard normal distribution to a normal one.
    x1 = mu + (sigma * z1)
    x2 = mu + (sigma * z2)
    return x1, x2
...
def main():
    ...
    '''
    # Generation of instances from the normal distribution
    ...
    empirical_variance = variance(values)
